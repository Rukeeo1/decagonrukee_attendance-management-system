$(document).ready(function(){
        ...
        success: function(response) {
           // console.log("Data: ", JSON.stringify(response));
            for (let i=0; i < response.length; i++){
                ...
                var markup = "<tr id='"+userId+"'><td>" + name + "</td><td>" +
                 email + "</td>" + 
                 `<td>
                 <input type ='radio' name='status' id='status' value='Absent' onchange="markAttendance('${email}','Absent');"/>Absent
                 <input type ='radio' name='status' id='status' value='Present' onchange="markAttendance('${email}','Present');"/>Present
                 </td>` + 
                 '<td><button class="ojigbo" id="tr_'+userId+'">Delete</button><td></tr>';
   ...
   function markAttendance(email,option){
